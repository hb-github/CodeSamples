...
export class ChartsComponent implements OnInit {
  ...
  private bubbleChart() {
    const bubbleChart = new Chart(document.getElementById('bubble-chart'), {
      type: 'bubble',
      data: {
        labels: 'Africa',
        datasets: [
  ...
  randomizeData() {
    const newDataBaby = this.exampleDataSet1.map(x =>
      Math.floor(Math.random() * 50)
    );
    const newdataSet2Baby = this.exampleDataSet2.map(x =>
